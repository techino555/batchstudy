...
一般的に変数設定、ログ設定と実処理を記載したバッチを実行するだけの
バッチファイルと実行したい処理を記載したバッチは分ける。
</br>
@fa[arrow-circle-down fa-lg]
+++
例えば前述のtest2.batを実行し、そのすべての処理ログを出力させるために
下記のようなバッチを実行する。このようにしないとtest2.batの実行結果を
ログ出力させる場合はすべての行に` > log.txt`などのログ出力の記載を
しないといけなくなるためである。
---
### 参照資料、ツールの紹介
---
